...
from incline.trend import spline_trend, trending
...
class TestRobustTrending:
    ...
    def test_trending_different_aggregations(self):
        ...
        for i in range(2):
            data = pd.DataFrame(
                {"value": [1, 2, 3, 4, 5] if i == 0 else [2, 3, 4, 5, 6]}
            )
            result = spline_trend(data)
            result["id"] = f"series_{i}"
            df_list.append(result)
    ...
    def test_trending_trim_fraction(self):
        ...
        for trim_frac in [0.1, 0.2, 0.3]:
            try:
                result = trending(
                    df_list, max_or_avg="trimmed_mean", trim_fraction=trim_frac
                )
                assert len(result) == 2
            except ImportError:
                pass  # Expected if scipy not available
    ...
    def test_trending_missing_derivative_order(self):
        ...
        trending([df_with_wrong_order], derivative_order=1)
    ...
    def test_robust_statistics_helpers(self):
        ...
        from incline.trend import (
            _compute_robust_statistics,
            _compute_weights,
            _outlier_fraction,
            _robust_skewness,
        )
